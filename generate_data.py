...
OUT_DIR = "/sessions/trusting-loving-gauss/mnt/outputs/data/raw"
...
with open(f"{OUT_DIR}/employees.csv","w",newline="") as f:
...
with open(f"{OUT_DIR}/tasks.csv","w",newline="") as f:
...
with open(f"{OUT_DIR}/time_entries.csv","w",newline="") as f:
...
for _ in range(12):
    months.append(m)
    if m.month == 12: m = date(m.year+1,1,1)
    else: m = date(m.year, m.month+1, 1)

# Burn-fraction targets per project (actual / planned total over the 12 months).
NEAR_DONE_PIDS = {2, 6, 10, 14, 17}     # ~65-75% burn
HEALTHY_PIDS   = {5, 7, 8, 12, 13, 19}  # ~40-55% burn
BEHIND_PIDS    = {3, 11, 16, 18, 20}    # ~78-92% burn
OVER_PIDS      = {1, 4, 9, 15}          # >100% burn (over budget)
...
with open(f"{OUT_DIR}/project_budget_actuals.csv","w",newline="") as f:
