...
DATA_DIR = Path("/sessions/trusting-loving-gauss/mnt/outputs/data/raw")
OUT_PATH = Path("/sessions/trusting-loving-gauss/mnt/outputs/project_tracker_dashboard.xlsx")
...
for p in projects:
    ...
    row = [
        ...
        round(bud["actual"], 2),
        # pct_complete (formula: done / total tasks for this project_id from Data_Tasks)
        None,  # placeholder — set below with formula
        # pct_budget_burned (formula): actual / budget
        None,
        # health_flag (formula): based on the two pcts
        None,
        # overrun (formula): actual - budget
        None,
    ]
    ws.append(row)

# Add formulas for the last 3 columns. Excel rows for projects start at 2.
proj_count = len(projects)
for r in range(2, 2 + proj_count):
    # pct_complete: COUNTIFS(Data_Tasks done for this pid) / COUNTIFS(total tasks for this pid)
    ws.cell(row=r, column=11, value=(
        f'=IFERROR(COUNTIFS(Data_Tasks!B:B,A{r},Data_Tasks!G:G,"Done")'
        f'/COUNTIFS(Data_Tasks!B:B,A{r}),0)'
    ))
    # pct_budget_burned = actual / budget
    ...
    # overrun = actual - budget (helper column used by Top 5 ranking)
...
# Conditional formatting on health_flag column (M)
...
# =================================================================
# Data_TimeEntries_Wk  (aggregated weekly)
# =================================================================
...
for (eid, wk), hrs in rows_sorted:
    e = emp_by_id[eid]
    ws.append([
        eid, e["full_name"], e["department"], wk, round(hrs,2), None
    ])
...
# Set good column widths for the KPI band
...
# ---- Section: Project health summary table ----
...
ws.cell(row=12, column=2).fill = HDR_FILL
ws.cell(row=12, column=3).fill = HDR_FILL
ws.cell(row=12, column=4).fill = HDR_FILL
...
# Conditional formatting on the Health column in this summary
ws.conditional_formatting.add(
...
# ---- Section: Top over-budget projects ----
...
# Top 5 by overrun, using the [overrun] helper column on tProjects.
# IF guard: if the rank-N overrun is <= 0, leave the row blank (fewer than 5 over-budget projects).
for i in range(5):
    r = 21 + i
    rank = i + 1
...
# Set wider cols I..K for the month summary
for c in ("I","J","K"): ws.column_dimensions[c].width = 13

# Section: Capacity hot list
...
# Build a unique-employee list with overallocated week count using a helper formula
# We'll drop in 12 rows of top performers via a sort/filter-style approach using helper functions.
# Simplest: list top 12 by overallocated week count using LARGE on a helper column.
# Helper column: in Data_TimeEntries_Wk, count overallocated weeks per employee_id via COUNTIFS.
# Here we just compute it directly with COUNTIFS over the time table.

# Build unique employees set
...
for i, eid in enumerate(unique_emp_ids):
    ...
    # Count + tiny tie-breaker (employee_id / 1,000,000) makes LARGE+MATCH unique.
    ws.cell(row=r, column=6, value=(
...
# Top 10 list pulled via LARGE/INDEX/MATCH from helper rows.
# INT() strips the tie-breaker decimal so the displayed week count is a whole number.
for i in range(10):
    r = 48 + i
    rank = i + 1
...
# Hide the helper rows visually by graying them
    ...
    for row in s.iter_rows():
        for cell in row:
            if cell.font and cell.font.name in (None, "Calibri"):
                # Preserve other formatting, just set name
                cell.font = Font(
                    name="Arial",
